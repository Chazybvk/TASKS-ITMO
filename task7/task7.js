...
console.log(board)
let ceil = document.querySelectorAll('.col')
let sim = ""
for (let item of ceil){
        for (f=0;f<board.length;++f){
            sim = board[f].column + board[f].position
            if (sim == item.dataset.numberString){
                let str = '<div>'
                str +='<p>' + board[f].description + '</p>'
                str += '</div>'
                item.innerHTML = str
            if(board[f].color == "white"){
